...
function GameManager(io, playersManager) {
    let self = this
    self.ramdomMax = 104
    self.autogame = true
    self.coefficient = 200
    self.position = 0
    ...
    self.algorithms = {
        ...
        game_info: {
            ...
            discharge_coins: 0,
            fee_taken: 3, // 3% of suction amount and move to profit pools
            suction_ratio: 0,
        },
        pools: {
            // discharge_pool: 0,
        ...
        applied_algorithm: null, // enum ["ADMIN_CRASH", "LOOPER_CRASH", SEED_ALGORITHM, SHARK_ALGORITHM, BALACE_ALGORITHM, SUCTION_ALGORITHM, DISCHARGE_ALGORITHM]
    }
            ...
            Game.find()
                ...
                .exec((er, games) => {
                    if (er || games == null) rj("Game not found")
                        ...
                        rs(
                            parseInt(
                                games.reduce((cur, next) => cur + next.bet, 0) /
                                    games.length
                            )
                        )
                    }
    ...
    const runGame = async () => {
        ...
        if (self.algorithms.is_admin_coefficient) {
            const _coefficient = await Setting.findOne({ name: "coefficient" })
    ...
    self.cashOut = (user, callback) => {
        ...
        let elapsed = new Date() - self.startTime
        let at = growthFunc(elapsed)
        let player = lib.getOwnProperty(playersManager.players, user.username)
            ...
            if (!er && doc != null) {
                if (doc.finances.free_coin > +betAmount) {
                    ...
                    playersManager
                        ...
                        .catch(err => {
                            return callback(err.message || "NOT_ENOUGH_MONEY")
                        })
                } else {
    ...
    self.getTotalBot = () => {
        const totalBots = Object.values(playersManager.players).filter(isBot)
            .length
    ...
    self.setupNextLooperBots = () => {
        ...
        self.algorithms.bots_config.current_looper =
            current_looper + 1 >= looper_bots.length ? 0 : current_looper + 1
    }
    ...
    self.setupCoefficient = async () => {
        ...
        if (game_auto && !game_auto.value) {
            ...
            if (coe && coe.value.length > 0) {
                self.coefficient =
                    coe.value[Math.floor(Math.random() * coe.value.length)] *
                    100
